...
const { coordinateTruckDashboardRecommendations } = await import('./truckDashboardRecommendations.ts');
const {
...
test('Biggest Opportunities displays Go LIVE first without changing the selected mission', () => {
  const mission = getTodaysMission(baseCommandCenter, [qrOpportunity, goLiveOpportunity]);
  const coordinated = coordinateTruckDashboardRecommendations(
    baseCommandCenter,
    [qrOpportunity, menuOpportunity, goLiveOpportunity],
    'truck-display-order',
    mission
  );

  assert.equal(coordinated.mission.sourceOpportunityId, qrOpportunity.id);
  assert.deepEqual(
    coordinated.opportunities.map(opportunity => opportunity.id),
    [goLiveOpportunity.id, menuOpportunity.id]
  );
});
