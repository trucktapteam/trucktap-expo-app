...
test('tolerant matching absorbs tiny float/string round-trip drift', () => {
  assert.ok(findPersistedRequestedLiveLocation([{
    truck_id: truckId,
    latitude: '38.25270004',
    longitude: '-85.75849996',
    label: requested.label.trim().toUpperCase(),
  }], truckId, requested), 'string-typed numeric values within epsilon should match, case-insensitively');
});

test('tolerant matching still rejects a genuinely different location', () => {
  assert.equal(findPersistedRequestedLiveLocation([{
    truck_id: truckId,
    latitude: requested.latitude + 0.01,
    longitude: requested.longitude,
    label: requested.label.trim(),
  }], truckId, requested), null);
});

test('findAnyPersistedLocationForTruck returns the best available row when nothing matches exactly', () => {
  const staleRow = {
    truck_id: truckId,
    latitude: 38,
    longitude: -85,
  ...
  assert.equal(findPersistedRequestedLiveLocation([staleRow], truckId, requested), null);
  assert.equal(findAnyPersistedLocationForTruck([staleRow], truckId), staleRow);
  assert.equal(findAnyPersistedLocationForTruck([staleRow], 'some-other-truck'), null);
  assert.equal(findAnyPersistedLocationForTruck(null, truckId), null);
});
