...
const stripped = transpiled
  ...
  .replace(/const releasePolicy_1 = require\("@\/lib\/releasePolicy"\);/, '')
...
test('maps a stop location status, distinguishing never-attempted from failed', () => {
  const neverAttempted = mapUpcomingStopLocationStatus({ stop_id: 'stop-b' });
  assert.equal(neverAttempted.latitude, null);
  assert.equal(neverAttempted.geocodeAttemptedAt, null);
  assert.equal(neverAttempted.geocodeFailedAt, null);

  const failed = mapUpcomingStopLocationStatus({
    stop_id: 'stop-c',
    location_geocode_attempted_at: '2026-07-27T12:00:00Z',
    location_geocode_failed_at: '2026-07-27T12:00:00Z',
  });
  assert.equal(failed.latitude, null);
  assert.equal(failed.geocodeFailedAt, '2026-07-27T12:00:00Z');

  const resolved = mapUpcomingStopLocationStatus({
    stop_id: 'stop-d',
    latitude: 40.7128,
    longitude: -74.006,
  ...
  assert.equal(resolved.latitude, 40.7128);
  assert.equal(resolved.geocodeFailedAt, null);
});
